# $Id: CDDB.pm,v 1.24 2001/05/03 20:30:38 troc Exp troc $
...
# Determine whether we can submit changes by e-mail.  TODO: HTTP
# submissions.
...
# Send a command.  If we're not connected, try to connect
# first. Returns 1 if the command is sent ok; 0 if there was a
# problem.
...
matching a given cddbp ID, track offsets, and total playing time.  It
accepts these in the same format returned by calculate_id(), so it's
easy to pass the values from one function to the other.
